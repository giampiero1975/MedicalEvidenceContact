                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @else
                        ...
                        @if (Auth::user()->role === 'business')
                            <x-nav-link href="{{ route('business.profile.edit') }}" :active="request()->routeIs('business.profile.*')">
                                Profilo struttura
                            </x-nav-link>
                            <x-nav-link href="{{ route('business.locations.index') }}" :active="request()->routeIs('business.locations.*')">
                                Sedi
                            </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if (Auth::user()->role === 'admin')
                ...
                <x-responsive-nav-link href="{{ route('admin.users.index') }}" :active="request()->routeIs('admin.users.*')">Utenti</x-responsive-nav-link>
                <x-responsive-nav-link href="{{ route('admin.job-postings.index') }}" :active="request()->routeIs('admin.job-postings.*')">Annunci</x-responsive-nav-link>
            @else
                <x-responsive-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')">{{ __('Dashboard') }}</x-responsive-nav-link>
                <x-responsive-nav-link href="{{ route('job-postings.index') }}" :active="request()->routeIs('job-postings.*')">Annunci</x-responsive-nav-link>
                <x-responsive-nav-link href="{{ route('interviews.index') }}" :active="request()->routeIs('interviews.*')">Colloqui</x-responsive-nav-link>
                @if (Auth::user()->role === 'professional')
                    <x-responsive-nav-link href="{{ route('professional.moodle.index') }}" :active="request()->routeIs('professional.moodle.*')">I miei Moodle</x-responsive-nav-link>
                @endif
                @if (Auth::user()->role === 'business')
                    <x-responsive-nav-link href="{{ route('business.profile.edit') }}" :active="request()->routeIs('business.profile.*')">Profilo struttura</x-responsive-nav-link>
                    <x-responsive-nav-link href="{{ route('business.locations.index') }}" :active="request()->routeIs('business.locations.*')">Sedi</x-responsive-nav-link>
                    <x-responsive-nav-link href="{{ route('business-points-of-contact.index') }}" :active="request()->routeIs('business-points-of-contact.*')">Referenti</x-responsive-nav-link>
                @endif
            @endif
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="flex items-center px-4">
                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                    <div class="shrink-0 me-3"><img class="size-10 rounded-full object-cover" src="{{ Auth::user()->profile_photo_url }}" alt="{{ Auth::user()->name }}" /></div>
                @endif
                <div>
