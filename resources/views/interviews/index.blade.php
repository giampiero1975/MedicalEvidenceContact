        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
            <div class="space-y-4">
                <x-ui.card>
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <h3 class="text-base font-semibold text-slate-950">Candidature da pianificare</h3>
                            <p class="mt-1 text-sm text-slate-600">Apri una candidatura per proporre uno o più slot di disponibilità.</p>
                        </div>
            ...
            <aside class="space-y-4">
                <x-ui.card>
                    ...
                    <p class="mt-2 text-sm text-slate-600">Dalla scheda candidatura puoi proporre più slot. Quando il professionista ne sceglie uno, dovrai confermarlo o rifiutarlo.</p>
    ...
    @else
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
            <div class="space-y-4">
                ...
                <x-ui.card>
                    <h3 class="text-base font-semibold text-slate-950">Slot disponibili e colloqui</h3>
                    <div class="mt-4 space-y-4">
                        @forelse ($interviews as $interview)
                            <article class="rounded-xl border border-slate-200 p-4">
                                <div class="flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                        <p class="font-semibold text-slate-950">{{ $interview->jobApplication->jobPosting->title }}</p>
                                        <p class="mt-1 text-sm text-slate-600">{{ $interview->scheduled_at->format('d/m/Y H:i') }} · {{ $interview->modeLabel() }}</p>
                                ...
                                @elseif ($interview->status === \App\Models\Interview::STATUS_ACCEPTED)
                                    <div class="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                                        <p class="text-sm font-semibold text-emerald-900">Colloquio confermato</p>
                                        @if ($interview->unlocksContacts())
                                            @php($business = $interview->jobApplication->jobPosting->owner)
                                            <p class="mt-2 text-sm text-emerald-900">Contatti sbloccati: {{ $business->email }}@if($business->phone) · {{ $business->phone }}@endif</p>
                                        @endif
                                    </div>
